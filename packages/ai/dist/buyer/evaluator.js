...
exports.evaluateBuyerDecision = evaluateBuyerDecision;
const groq_js_1 = require("../groq.js");
async function evaluateBuyerDecision(product, policy, apiKey) {
    const violations = [];
    // 1. Currency validation
    if (product.currency.toUpperCase() !== policy.currency.toUpperCase()) {
        violations.push(`Currency mismatch: product is in ${product.currency} but buyer policy requires ${policy.currency}`);
    }
    // 2. Budget / Max price constraint
    if (product.price > policy.maxPrice) {
        violations.push(`Price ₹${(product.price / 100).toFixed(2)} exceeds maximum buyer budget of ₹${(policy.maxPrice / 100).toFixed(2)}`);
    }
    ...
    if (policy.blockedKeywords && policy.blockedKeywords.length > 0) {
        for (const blocked of policy.blockedKeywords) {
            if (textToScan.includes(blocked.toLowerCase())) {
                violations.push(`Product contains blocked keyword: "${blocked}"`);
            }
        }
    }
    // 4. Required keywords check
    if (policy.requiredKeywords && policy.requiredKeywords.length > 0) {
        const hasRequiredKeyword = policy.requiredKeywords.some((keyword) => textToScan.includes(keyword.toLowerCase()));
        if (!hasRequiredKeyword) {
            violations.push(`Product does not match any required buyer keywords: [${policy.requiredKeywords.join(", ")}]`);
        }
    }
    // 5. Merchant preferences check
    if (policy.preferredMerchantIds &&
        policy.preferredMerchantIds.length > 0 &&
        product.merchantId) {
        if (!policy.preferredMerchantIds.includes(product.merchantId)) {
            violations.push(`Merchant ${product.merchantId} is not in the buyer's approved merchant list`);
        }
    }
    const policyPassed = violations.length === 0;
    // 6. Autonomous LLM Evaluation with Groq
    const client = (0, groq_js_1.createGroqClient)(apiKey);
    if (client) {
        try {
            const completion = await client.chat.completions.create({
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are an autonomous AI buyer evaluating whether to purchase a commerce item based on merchant data and buyer constraints. Output strictly valid JSON with keys: decision ('BUY' or 'SKIP'), reason (detailed string explaining purchase suitability), confidence (number between 0.8 and 1.0).",
                    },
                    {
                        role: "user",
