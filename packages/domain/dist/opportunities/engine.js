...
exports.detectLowStockOpportunities = detectLowStockOpportunities;
exports.detectHighDemandOpportunities = detectHighDemandOpportunities;
exports.detectCrossSellOpportunities = detectCrossSellOpportunities;
exports.detectLowConversionOpportunities = detectLowConversionOpportunities;
exports.getRevenueOpportunities = getRevenueOpportunities;
...
async function detectLowStockOpportunities(merchantId, threshold = 5) {
    ...
    for (const variant of variants) {
        ...
        const recentSales = variant.orderItems.reduce((sum, item) => sum + item.quantity, 0);
        // Rule: available stock <= threshold AND has paid sales
        if (availableStock <= threshold && recentSales > 0) {
            const priority = availableStock <= 2 ? "HIGH" : "MEDIUM";
...
async function detectHighDemandOpportunities(merchantId, threshold = 5) {
    const variants = await database_1.prisma.productVariant.findMany({
        where: {
            product: {
                merchantId,
    ...
    for (const variant of variants) {
        ...
        if (sales >= threshold) {
            const priority = sales >= 10 ? "HIGH" : "MEDIUM";
            opportunities.push({
...
async function detectCrossSellOpportunities(merchantId, minOccurrence = 2) {
    const paidOrders = await database_1.prisma.order.findMany({
        where: {
            merchantId,
            status: {
                in: ["PAID", "FULFILLED"],
    ...
    for (const order of paidOrders) {
        ...
        const productMapInOrder = new Map();
    ...
    for (const [, pair] of pairMap.entries()) {
        if (pair.count >= minOccurrence) {
            const priority = pair.count >= 4 ? "HIGH" : "MEDIUM";
            opportunities.push({
...
async function detectLowConversionOpportunities(merchantId) {
    ...
    for (const product of products) {
        const totalPaidSales = product.variants.reduce((sum, v) => sum +
            v.orderItems.reduce((iSum, item) => iSum + item.quantity, 0), 0);
...
async function getRevenueOpportunities(merchantId, options) {
    const [lowStock, highDemand, crossSell, lowConversion] = await Promise.all([
    ...
    return allOpportunities.sort((a, b) => PRIORITY_WEIGHTS[b.priority] - PRIORITY_WEIGHTS[a.priority]);
}
