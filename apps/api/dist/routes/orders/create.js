...
exports.default = createOrderRoute;
...
async function createOrderRoute(app) {
    app.post("/orders", {
            ...
            const order = await database_1.prisma.$transaction(async (tx) => {
                ...
                const orderItems = items.map((item) => {
                    ...
                    if (available < item.quantity) {
                        throw new Error(`Insufficient inventory for ${variant.sku}`);
                    }
                ...
                for (const item of items) {
                    ...
                    if (updated.count !== 1) {
                        throw new Error(`Unable to reserve inventory for variant ${item.variantId}`);
                    }
                }
                return createdOrder;
            });
